// ═══════════════════════════════════════════════════════════
// SILSILAH BANI SEBIL — app.js
// Versi: 2.0 — Rebuilt with proper error handling & standards
// ═══════════════════════════════════════════════════════════
...
// ── LOCAL STORAGE ─────────────────────────────────────────
function getPending(){
  try{ return JSON.parse(localStorage.getItem('ikbas_pending')||'[]'); }
  catch(e){ return []; }
}
function savePending(arr){
  try{ localStorage.setItem('ikbas_pending', JSON.stringify(arr)); }
  catch(e){ console.error('savePending error:', e); }
}
...
function checkPassword(){
  ...
  if(_chk(val)){
    ...
    sessionStorage.setItem('ikbas_admin_pw', val);
    enterApp();
  } else {
...
function initAdminMode(){
  ...
  if(hst){
    ...
    hst.insertBefore(badge, hst.firstChild);
  }
  ...
  if(pending.length > 0){
    setTimeout(function(){ showToast('🔔 Ada ' + pending.length + ' permintaan menunggu!'); }, 1500);
  }
}
...
function openAdminPanel(){
  ...
  var adminPw = sessionStorage.getItem('ikbas_admin_pw') || '';
  ...
  .then(function(d){
    if(d.success){
      ...
      var local = getPending();
      var ids = d.data.map(function(p){ return p.id || p.timestamp; });
      local.forEach(function(p){
        if(ids.indexOf(p.id || p.timestamp) < 0) d.data.push(p);
      });
      renderAdminPanel(d.data);
    } else {
      contentEl.innerHTML = '<div style="color:#ef4444;padding:16px;font-weight:600">Error: ' + (d.error || 'Gagal load data') + '</div>';
    }
  })
  .catch(function(e){
    console.error('openAdminPanel error:', e);
...
function renderAdminPanel(pending){
  ...
  var html = pending.map(function(item, i){
    ...
    if(item.tipe === 'UPDATE' && item.namaAsli){
      currentNode = allNodes.find(function(n){ return n.n === item.namaAsli; });
    }
    var parentNode = null;
    if(item.namaOrangTua){
      var pname = item.namaOrangTua.split(' + ')[0].trim();
      parentNode = allNodes.find(function(n){ return n.n.toLowerCase() === pname.toLowerCase(); });
    }

    var h = '<div style="background:#fff;border:1.5px solid var(--green-mid);border-radius:12px;padding:14px;margin-bottom:12px;box-shadow:0 2px 8px rgba(26,122,60,0.08)">';
    h += '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">';
    h += '<span style="background:' + tipeColor + '22;color:' + tipeColor + ';border:1px solid ' + tipeColor + '44;border-radius:20px;padding:3px 10px;font-size:11px;font-weight:700">' + tipeIcon + ' ' + tipeLabel + '</span>';
    h += '<span style="font-size:10px;color:var(--txt3)">' + new Date(item.timestamp || Date.now()).toLocaleString('id-ID') + '</span>';
    h += '</div>';
    ...
    // BEFORE
    h += '<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:10px">';
    h += '<div style="font-size:10px;font-weight:700;color:#dc2626;text-transform:uppercase;letter-spacing:.4px;margin-bottom:6px">⬅ Sebelum</div>';
    if(item.tipe === 'TAMBAH_ANAK'){
      h += '<div style="font-size:11px;color:#6b7280;font-style:italic">Belum ada</div>';
      if(parentNode){ h += '<div style="font-size:11px;color:#374151;margin-top:4px">Orang tua: <b>' + parentNode.n + '</b></div>'; h += '<div style="font-size:10px;color:#6b7280">Saat ini punya ' + parentNode.c.length + ' anak</div>'; }
    } else if(item.tipe === 'UPDATE' && currentNode){
      h += '<div style="font-size:13px;font-weight:700;color:#374151">' + currentNode.n + '</div>';
      if(currentNode.s) h += '<div style="font-size:11px;color:#6b7280">♥ ' + currentNode.s + '</div>';
      if(currentNode.note) h += '<div style="font-size:10px;color:#6b7280;margin-top:2px">' + currentNode.note + '</div>';
      if(currentNode.foto) h += '<div style="margin-top:4px"><img src="' + currentNode.foto + '" style="width:36px;height:36px;border-radius:50%;object-fit:cover;border:2px solid #1a7a3c"></div>';
    } else if(item.tipe === 'HAPUS'){
      var delNode = allNodes.find(function(n){ return n.n === (item.namaAsli || item.nama); });
      if(delNode){ h += '<div style="font-size:13px;font-weight:700;color:#374151">' + delNode.n + '</div>'; h += '<div style="font-size:10px;color:#6b7280">' + delNode.c.length + ' keturunan akan ikut terhapus</div>'; }
      else { h += '<div style="font-size:11px;color:#6b7280">' + (item.namaAsli || item.nama) + '</div>'; }
    } else {
      h += '<div style="font-size:11px;color:#6b7280;font-style:italic">Data tidak ditemukan</div>';
    }
    h += '</div>';

    // AFTER
    h += '<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:10px">';
    h += '<div style="font-size:10px;font-weight:700;color:#059669;text-transform:uppercase;letter-spacing:.4px;margin-bottom:6px">➡ Sesudah</div>';
    if(item.tipe === 'HAPUS'){
      h += '<div style="font-size:11px;color:#dc2626;font-weight:600">Data akan dihapus permanen</div>';
    } else {
      h += '<div style="font-size:13px;font-weight:700;color:#374151">' + (item.nama || '-') + '</div>';
      if(item.pasangan) h += '<div style="font-size:11px;color:#6b7280">♥ ' + item.pasangan + '</div>';
      if(item.namaOrangTua) h += '<div style="font-size:11px;color:#374151;margin-top:4px">Orang tua: <b>' + item.namaOrangTua + '</b></div>';
      if(item.jk) h += '<div style="font-size:10px;color:#6b7280">' + (item.jk === 'L' ? 'Laki-laki' : 'Perempuan') + '</div>';
      if(item.tglLahir) h += '<div style="font-size:10px;color:#6b7280">Lahir: ' + item.tglLahir + (item.tmptLahir ? ' di ' + item.tmptLahir : '') + '</div>';
      if(item.hp) h += '<div style="font-size:10px;color:#6b7280">HP: ' + item.hp + '</div>';
      if(item.catatan) h += '<div style="font-size:10px;color:#6b7280;margin-top:2px">' + item.catatan + '</div>';
      if(item.fotoUrl) h += '<div style="margin-top:4px"><img src="' + item.fotoUrl + '" style="width:36px;height:36px;border-radius:50%;object-fit:cover;border:2px solid #1a7a3c"></div>';
    }
    h += '</div></div>';

    h += '<div style="display:flex;gap:8px">';
    h += '<button onclick="approveItem(' + i + ')" style="flex:1;background:var(--green);border:none;border-radius:8px;color:#fff;padding:9px;font-size:12px;font-weight:700;cursor:pointer;transition:opacity .2s" onmouseover="this.style.opacity=\'.85\'" onmouseout="this.style.opacity=\'1\'">✅ Setujui & Terapkan</button>';
    h += '<button onclick="rejectItem(' + i + ')" style="flex:1;background:#fff;border:1.5px solid #ef4444;border-radius:8px;color:#ef4444;padding:9px;font-size:12px;font-weight:700;cursor:pointer;transition:opacity .2s" onmouseover="this.style.opacity=\'.7\'" onmouseout="this.style.opacity=\'1\'">❌ Tolak</button>';
    h += '</div></div>';
    return h;
  }).join('');
...
function approveItem(idx){
  ...
  var btn = document.querySelectorAll('#adminContent button')[idx * 2];
  ...
  if(item.tipe === 'TAMBAH_ANAK' && item.namaOrangTua){
    ...
    if(pnode){
      var nid = Date.now();
      var nn = { id: nid, n: item.nama, s: item.pasangan || null, g: pnode.g + 1, w: pnode.w, note: item.catatan || null, foto: item.fotoUrl || null, c: [], _parent: pnode };
  ...
  var local = getPending().filter(function(p){ return (p.id || p.timestamp) !== (item.id || item.timestamp); });
  savePending(local);
  ...
  .then(function(d){
    ...
    else { showToast('Error server: ' + (d.error || 'Unknown')); }
  })
  .catch(function(e){
    console.error('approveItem error:', e);
    showToast('Disetujui di browser. Gagal sync server: ' + e.message);
  });
...
function rejectItem(idx){
  ...
  fetch('/api/approve', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-admin-password': adminPw },
    body: JSON.stringify({ action: 'reject', item: item })
  })
  .then(function(r){ return r.json(); })
  .then(function(d){ showToast(d.message || 'Permintaan ditolak.'); })
  .catch(function(e){ console.error('rejectItem error:', e); showToast('Permintaan ditolak (offline).'); });
...
function uploadPhoto(file){
  ...
  var uniqueName = 'bani-sebil-' + Date.now() + '-' + Math.random().toString(36).substr(2, 9);
  ...
  if(!CLOUDINARY_CLOUD || CLOUDINARY_CLOUD === 'CLOUD_NAME_ANDA'){
    ...
    reader.onload = function(e){
      ...
      document.getElementById('fFotoUrl').value = ''; // Don't save base64
      document.getElementById('photoUploading').style.display = 'none';
      showToast('Preview tampil! Isi CLOUDINARY_CLOUD di app.js untuk upload permanen.');
    };
  ...
  formData.append('public_id', uniqueName); // Unique ID prevents overwrite
  ...
  .then(function(d){
    document.getElementById('photoUploading').style.display = 'none';
    if(d.secure_url){
      // Auto face-crop for profile photo
      ...
      console.log('Cloudinary upload success:', d.secure_url);
...
function setupPhotoListeners(){
  ['fNama','fPasangan'].forEach(function(id){
...
function nColor(n){
  if(n.g <= 3) return ACOL;
  if(n.g === 4) return SCOL;
  if(n.w === 0) return WCOL[0];
  if(n.w === 1) return WCOL[1];
  if(n.w === 2) return WCOL[2];
  return '#7c3aed';
}
...
var vx = 0, vy = 0, vk = 0.6;
var dragging = false, dragSX = 0, dragSY = 0, dragVX = 0, dragVY = 0;
...
function buildIndex(node, parent){
  ...
  if(node.g > maxGen) maxGen = node.g;
  (node.c || []).forEach(function(c){ buildIndex(c, node); });
}
...
function layoutTree(){
  ...
  function assignX(node){
    var kids = collapsed.has(node.id) ? [] : (node.c || []);
    if(!kids.length){ posMap[node.id] = { x: counter * (NW + HGAP), y: genY[node.g] || 0 }; counter++; return; }
    kids.forEach(function(c){ assignX(c); });
    var fx = posMap[kids[0].id].x, lx = posMap[kids[kids.length-1].id].x;
    posMap[node.id] = { x: (fx + lx) / 2, y: genY[node.g] || 0 };
  }
  TREE.ancestors.forEach(function(a, i){ posMap[a.id] = { x: 0, y: genY[a.g] || i * (NH + VGAP) }; });
  counter = 0; assignX(TREE.sebil);
  var sx = posMap[TREE.sebil.id].x;
  TREE.ancestors.forEach(function(a){ posMap[a.id].x = sx; });
}
...
function render(){
  ...
  function drawLinks(node){
    ...
    var sp = posMap[node.id]; if(!sp) return;
    (node.c || []).forEach(function(child){
      var cp = posMap[child.id]; if(!cp) return;
      var x1 = sp.x + NW/2, y1 = sp.y + NH, x2 = cp.x + NW/2, y2 = cp.y, my = (y1 + y2) / 2;
      var p = document.createElementNS(ns, 'path');
      p.setAttribute('d', 'M' + x1 + ',' + y1 + ' C' + x1 + ',' + my + ' ' + x2 + ',' + my + ' ' + x2 + ',' + y2);
      p.setAttribute('fill', 'none'); p.setAttribute('stroke', nColor(child) + '50'); p.setAttribute('stroke-width', '2');
      linkG.appendChild(p); drawLinks(child);
    });
  }

  for(var i = 0; i < TREE.ancestors.length - 1; i++){
    var a = TREE.ancestors[i], b = TREE.ancestors[i+1], ap = posMap[a.id], bp = posMap[b.id];
    if(ap && bp){ var lp = document.createElementNS(ns, 'path'); lp.setAttribute('d', 'M' + (ap.x+NW/2) + ',' + (ap.y+NH) + ' L' + (bp.x+NW/2) + ',' + bp.y); lp.setAttribute('fill', 'none'); lp.setAttribute('stroke', ACOL + '60'); lp.setAttribute('stroke-width', '2'); linkG.appendChild(lp); }
  }
  ...
  function drawNode(node){
    ...
    g.addEventListener('click', function(e){
      e.stopPropagation();
      if(document.getElementById('relbox').classList.contains('show') && relActive > 0) selectForRelCalc(node);
      else showInfo(node.id);
    });
    ...
    sh.setAttribute('x', '2'); sh.setAttribute('y', '3'); sh.setAttribute('width', NW); sh.setAttribute('height', NH);
    sh.setAttribute('rx', '11'); sh.setAttribute('fill', 'rgba(26,122,60,0.07)'); sh.setAttribute('filter', 'url(#cardshadow)');
    g.appendChild(sh);

    // Card
    var bg = document.createElementNS(ns, 'rect');
    bg.setAttribute('width', NW); bg.setAttribute('height', NH); bg.setAttribute('rx', '11');
    bg.setAttribute('fill', isHL ? col + '18' : '#ffffff');
    bg.setAttribute('stroke', isHL ? col : col + '45');
    bg.setAttribute('stroke-width', isHL ? '2.5' : '1.5');
    g.appendChild(bg);

    // Left bar
    var bar = document.createElementNS(ns, 'rect');
    bar.setAttribute('width', '5'); bar.setAttribute('height', NH - 10);
    bar.setAttribute('x', '0'); bar.setAttribute('y', '5'); bar.setAttribute('rx', '2.5');
    bar.setAttribute('fill', col); g.appendChild(bar);

    // Glow if selected
    if(isHL){
      var gl = document.createElementNS(ns, 'rect');
      gl.setAttribute('width', NW); gl.setAttribute('height', NH); gl.setAttribute('rx', '11');
      gl.setAttribute('fill', 'none'); gl.setAttribute('stroke', col); gl.setAttribute('stroke-width', '2.5');
    ...
    nt.setAttribute('x', '12'); nt.setAttribute('y', node.s ? '18' : '28');
    nt.setAttribute('fill', '#0d2b18'); nt.setAttribute('font-size', isAnc ? '12px' : '11px');
    nt.setAttribute('font-weight', '700'); nt.setAttribute('font-family', 'Plus Jakarta Sans,Inter,system-ui,sans-serif');
    nt.textContent = nm; g.appendChild(nt);

    // Spouse
    if(node.s){
      var spStr = node.s.length > 22 ? node.s.slice(0, 21) + '...' : node.s;
      var st = document.createElementNS(ns, 'text');
      st.setAttribute('x', '12'); st.setAttribute('y', '33');
      st.setAttribute('fill', '#3d6b4f'); st.setAttribute('font-size', '10px');
      st.setAttribute('font-family', 'Inter,system-ui,sans-serif');
      st.textContent = '♥ ' + spStr; g.appendChild(st);
    }

    // Badge
    if(kids.length > 0){
      var cc = kids.length, bw = cc > 9 ? 28 : 22, bx = NW - bw - 4, by = NH - 16;
      var br = document.createElementNS(ns, 'rect');
      br.setAttribute('x', bx); br.setAttribute('y', by); br.setAttribute('width', bw); br.setAttribute('height', 13);
      br.setAttribute('rx', '6'); br.setAttribute('fill', isColl ? col : 'rgba(26,122,60,0.1)'); g.appendChild(br);
      var bt = document.createElementNS(ns, 'text');
      bt.setAttribute('x', bx + bw/2); bt.setAttribute('y', by + 9.5); bt.setAttribute('text-anchor', 'middle');
      bt.setAttribute('fill', isColl ? '#fff' : col); bt.setAttribute('font-size', '9px'); bt.setAttribute('font-weight', '800');
      bt.setAttribute('font-family', 'Inter,system-ui,sans-serif');
      bt.textContent = isColl ? '+' + cc : cc; g.appendChild(bt);
    }

    // Foto avatar (top-right corner)
    if(node.foto){
      try{
      ...
      } catch(e){ console.warn('Foto render error:', e); }
    }
  ...
  TREE.ancestors.forEach(function(a){ drawNode(a); });
  drawNode(TREE.sebil);
...
svg.addEventListener('wheel', function(e){
  ...
  var f = e.deltaY < 0 ? 1.15 : 0.87, nk = Math.max(0.05, Math.min(3, vk * f));
  vx = mx - (mx - vx) * (nk / vk); vy = my - (my - vy) * (nk / vk); vk = nk; applyTransform();
}, { passive: false });

var lastTD = null, tSX = 0, tSY = 0, tVX = 0, tVY = 0;
svg.addEventListener('touchstart', function(e){
  if(e.touches.length === 1){ dragging = true; tSX = e.touches[0].clientX; tSY = e.touches[0].clientY; tVX = vx; tVY = vy; lastTD = null; }
...
svg.addEventListener('touchmove', function(e){
  e.preventDefault();
  if(e.touches.length === 1 && dragging){ vx = tVX + (e.touches[0].clientX - tSX); vy = tVY + (e.touches[0].clientY - tSY); applyTransform(); }
  else if(e.touches.length === 2 && lastTD){
    var dx = e.touches[0].clientX - e.touches[1].clientX, dy = e.touches[0].clientY - e.touches[1].clientY;
    var dist = Math.sqrt(dx*dx + dy*dy), scale = dist / lastTD;
    var nk = Math.max(0.05, Math.min(3, vk * scale));
    vx = tSX - (tSX - tVX) * (nk / vk); vy = tSY - (tSY - tVY) * (nk / vk); vk = nk;
    lastTD = dist; tVX = vx; tVY = vy; applyTransform();
  }
}, { passive: false });
...
function toggleCollapse(id){
  if(collapsed.has(id)){ collapsed.delete(id); var node = nodeMap[id]; (node.c || []).forEach(function(c){ if((c.c || []).length > 0) collapsed.add(c.id); }); showToast('Dibuka 1 level'); }
  else { collapsed.add(id); showToast('Cabang dilipat'); }
  layoutTree(); render();
}
...
function showInfo(id){
  ...
  h += '<div class="igb">' + gLabel(node.g) + '</div>';
  h += '<div class="iname">' + node.n + '</div>';
  if(node.s) h += '<div class="isp">Pasangan: <span>' + node.s + '</span></div>';
  if(node.note) h += '<div class="inote">📍 ' + node.note + '</div>';

  // Extra fields
  if(node.jk || node.tglLahir || node.tmptLahir || node.hp || node.alamat){
    h += '<div class="idv"></div>';
  ...
  if(kids.length > 0){
    h += '<div class="idv"></div><div style="font-size:10px;color:var(--txt3);margin-bottom:4px;font-weight:700;text-transform:uppercase;letter-spacing:.4px">Anak-anak (' + kids.length + ')</div><div class="icl">';
    kids.slice(0, 15).forEach(function(c, i){ h += '<div class="ici" onclick="navigateTo(' + c.id + ',true)">' + (i+1) + '. ' + c.n + (c.s ? ' + ' + c.s : '') + '</div>'; });
    if(kids.length > 15) h += '<div style="font-size:10px;color:var(--txt3);padding:3px 8px">...dan ' + (kids.length-15) + ' lainnya</div>';
    h += '</div>';
  }

  h += '<div class="idv"></div><div style="display:flex;gap:6px">';
  h += '<button onclick="openEditNode(' + id + ')" style="flex:1;background:var(--green-light);border:1.5px solid var(--green-mid);border-radius:8px;color:var(--green);padding:7px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">✏️ Update</button>';
  h += '<button onclick="openAddChild(' + id + ')" style="flex:1;background:var(--yellow-light);border:1.5px solid rgba(245,197,24,0.4);border-radius:8px;color:#7a6000;padding:7px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">➕ Tambah Anak</button>';
  h += '</div>';
  if(isAdmin){
    h += '<div style="display:flex;gap:6px;margin-top:6px">';
    h += '<button onclick="moveNode(' + id + ')" style="flex:1;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:8px;color:#1d4ed8;padding:7px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">↔️ Pindahkan</button>';
    h += '<button onclick="deleteNode(' + id + ')" style="flex:1;background:#fef2f2;border:1.5px solid #fecaca;border-radius:8px;color:#dc2626;padding:7px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">🗑️ Hapus</button>';
    h += '</div>';
  }
...
function navigateTo(id, showPanel){
  ...
  var tx = w/2 - (pos.x + NW/2) * vk, ty = h/2 - (pos.y + NH/2) * vk;
...
// ── SEARCH ────────────────────────────────────────────────
var stimer;
document.getElementById('si').addEventListener('input', function(){ clearTimeout(stimer); stimer = setTimeout(function(){ doSearch(document.getElementById('si').value); }, 200); });
...
function doSearch(q){
  ...
  q = q.toLowerCase();
  var hits = allNodes.filter(function(n){ return n.n.toLowerCase().indexOf(q) >= 0 || (n.s && n.s.toLowerCase().indexOf(q) >= 0); }).slice(0, 18);
  if(!hits.length){ res.innerHTML = '<div class="sri" style="color:var(--txt3)">Tidak ditemukan</div>'; res.classList.add('show'); return; }
  res.innerHTML = hits.map(function(n){ return '<div class="sri" onclick="pickSearch(' + n.id + ')">' + n.n + (n.s ? ' <span style="color:var(--txt3)">+ ' + n.s + '</span>' : '') + '<span class="gb">' + gLabel(n.g) + '</span></div>'; }).join('');
...
function buildSidebar(){
  ...
  var h = '<div class="bi" onclick="navigateTo(' + TREE.sebil.id + ',true)"><div class="bn"><div class="bd" style="background:#f59e0b"></div>Leluhur & Bapak Sebil</div><div class="bm">Rantai leluhur dari Raden Wirawangsa</div></div>';
  (TREE.sebil.c || []).forEach(function(n){ var col = nColor(n), dc = countDesc(n); h += '<div class="bi" id="bi' + n.id + '" onclick="focusBranch(' + n.id + ')"><div class="bn"><div class="bd" style="background:' + col + '"></div>' + n.n + '</div><div class="bm">' + (n.s ? '+ ' + n.s + ' · ' : '') + dc + ' keturunan</div></div>'; });
  bl.innerHTML = h;
}
...
function buildListView(){
  var byGen = {}; allNodes.forEach(function(n){ if(!byGen[n.g]) byGen[n.g] = []; byGen[n.g].push(n); });
  var h = '';
  Object.keys(byGen).sort(function(a,b){ return +a - +b; }).forEach(function(g){ var list = byGen[g]; h += '<div class="lgs"><div class="lgt">' + gLabel(+g) + ' <span class="lgc">' + list.length + ' orang</span></div><div class="lgr">' + list.map(function(n){ return '<div class="lc" onclick="pickList(' + n.id + ')"><div class="lcn">' + n.n + '</div>' + (n.s ? '<div class="lcs">♥ ' + n.s + '</div>' : '') + '</div>'; }).join('') + '</div></div>'; });
...
function submitForm(){
  ...
  .catch(function(e){
    ...
    // Fallback to localStorage
    var pending = getPending(); pending.push(payload); savePending(pending);
    btn.innerHTML = '📤 Kirim Data'; btn.disabled = false;
    closeModal();
    showToast('⚠️ Tersimpan lokal (server tidak tersedia). Admin akan memverifikasi.');
  });
}
...
function deleteNode(id){
  ...
  msg += '\n\nPermintaan ini akan dikirim ke admin untuk diverifikasi.';
  ...
  fetch('/api/submit', {
    ...
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      tipe: 'HAPUS', nama: nodeName, namaAsli: nodeName,
  ...
  .then(function(r){ return r.json(); })
  .then(function(d){ showToast(d.success ? '"' + nodeName + '" dihapus. Admin akan memverifikasi penghapusan permanen.' : 'Dihapus dari tampilan. Gagal kirim ke server.'); })
  .catch(function(e){ console.error('deleteNode error:', e); showToast('"' + nodeName + '" dihapus dari tampilan.'); });
}
...
function moveNode(id){
  ...
  updateGen(node, newParent.g + 1);
  layoutTree(); render(); buildSidebar(); showInfo(id);
...
function calcRelation(){
  if(!relNode1 || !relNode2){ showToast('Pilih dua orang terlebih dahulu'); return; }
  if(relNode1.id === relNode2.id){ showResult('Orang yang sama', '', 'Anda memilih orang yang sama', ''); return; }
  var lcas = findLCAs(relNode1, relNode2);
  if(!lcas.length){ showResult('Tidak ada hubungan', '', 'Kedua orang tidak memiliki leluhur yang sama', ''); return; }
  var info = buildRelInfo(lcas[0].d1, lcas[0].d2, lcas[0].lca);
  var extraPath = info.path;
  if(lcas.length > 1){ extraPath += '\n\nADA PERNIKAHAN DALAM KELUARGA\nAda ' + lcas.length + ' jalur hubungan:\n'; lcas.forEach(function(lca, i){ var r = buildRelInfo(lca.d1, lca.d2, lca.lca); extraPath += (i+1) + '. ' + r.rel + ' (via ' + lca.lca.n + ', jarak ' + lca.d1 + '+' + lca.d2 + '=' + lca.total + ')' + (i === 0 ? ' - TERDEKAT' : '') + '\n'; }); extraPath += '\nHubungan nomor 1 adalah yang paling relevan.'; }
  showResult(info.rel + (lcas.length > 1 ? ' (terdekat)' : ''), info.sen, info.desc, extraPath);
}
...
document.addEventListener('keydown', function(e){
  if(e.key === 'Escape'){ closeInfo(); closeModal(); var rb = document.getElementById('relbox'); if(rb && rb.classList.contains('show')) toggleRelCalc(); }
...
setupPhotoListeners();
if(window.innerWidth <= 768){ sideOpen = false; document.getElementById('side').classList.add('off'); }
