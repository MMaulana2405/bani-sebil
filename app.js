...
function openAdminPanel(){
  ...
  contentEl.innerHTML='<div style="text-align:center;padding:20px">Memuat...</div>';
  var token=GITHUB_TOKEN;
  var hasToken=token&&token.length>20&&token.indexOf('BAGIAN')<0;
  if(!hasToken){renderAdminPanel(getPending());return;}
  fetch('https://api.github.com/repos/'+GITHUB_OWNER+'/'+GITHUB_REPO+'/contents/data/submissions.json',
    {headers:{'Authorization':'token '+token,'Accept':'application/vnd.github.v3+json'}})
  .then(function(r){return r.json();})
  .then(function(fd){
    var gp=[];
    try{gp=JSON.parse(atob(fd.content.split('\n').join('')));}catch(e){}
    var local=getPending();
    var ids=local.map(function(p){return p.id||p.timestamp;});
    gp.forEach(function(p){if(ids.indexOf(p.id||p.timestamp)<0)local.push(p);});
    renderAdminPanel(local);
  }).catch(function(){renderAdminPanel(getPending());});
}
...
function approveItem(idx){
  var pending=getPending();var item=pending[idx];if(!item)return;
...
function rejectItem(idx){
  ...
  var pending=getPending();var item=pending[idx];
  pending.splice(idx,1);savePending(pending);
  removeFromGitHub(item);
...
function removeFromGitHub(item){
  ...
  .then(function(fd){
    ...
    return fetch(apiUrl,{method:'PUT',
      ...
      body:JSON.stringify({message:'Update submissions',content:btoa(unescape(encodeURIComponent(JSON.stringify(cur,null,2)))),sha:fd.sha})
    });
  }).catch(function(){});
}
...
function openEditNode(id){var node=nodeMap[id];if(!node)return;editingNodeId=id;addingToParentId=null;document.getElementById('modalTitle').textContent='Update: '+node.n;document.getElementById('fNama').value=node.n;document.getElementById('fPasangan').value=node.s||'';['fJK','fTgl','fTmpt','fHP','fAlamat'].forEach(function(id2){document.getElementById(id2).value='';});document.getElementById('fCatatan').value=node.note||'';document.getElementById('fParentInfo').style.display='none';document.getElementById('editModal').classList.add('show');}
...
function submitForm(){
  ...
  var payload={tipe:editingNodeId?'UPDATE':(addingToParentId?'TAMBAH_ANAK':'TAMBAH_BARU'),nama:nama,pasangan:document.getElementById('fPasangan').value.trim(),jk:document.getElementById('fJK').value,tglLahir:document.getElementById('fTgl').value,tmptLahir:document.getElementById('fTmpt').value.trim(),hp:document.getElementById('fHP').value.trim(),alamat:document.getElementById('fAlamat').value.trim(),catatan:document.getElementById('fCatatan').value.trim(),namaOrangTua:parentNode?(parentNode.n+(parentNode.s?' + '+parentNode.s:'')):'',namaAsli:editNode?editNode.n:'',timestamp:new Date().toISOString(),id:'sub_'+Date.now()};
